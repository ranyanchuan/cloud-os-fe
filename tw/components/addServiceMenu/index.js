	...
	render() {
				...
				title={
					<div className={`${added ? "" : node} ${item.newAdded ? red : ''}`}>{item.name}
						<div className={option} >
							<span onClick={this.addNode.bind(this, item, 'forward')}>之前</span>
              &nbsp;&nbsp;
              <span style={{ marginRight: 20 }} onClick={this.addNode.bind(this, item, 'backward')}>之後</span>
						</div>
		...
		return (
			<div className={wrapper}>
				...
				<footer>
					<ButtonU8cDefault onClick={this.cancel}>取消</ButtonU8cDefault>
					<ButtonU8cPrimary id="saveMenuId" disabled={!added}>保存</ButtonU8cPrimary>
				</footer>
			</div>
		);
	}
