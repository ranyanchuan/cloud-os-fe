...
class AddApp extends Component {
	...
	getAppRef = () => {
		...
		getAppRef().then((result) => {
			...
			if (!applicationId) {
				...
				if (!match) {
					throw new Error("錯誤的參數：applicationId");
				}
			}
	...
	servicePublish = () => {
		...
		if (serviceType == 1 && serviceName.trim().replace(/[^\x00-\xff]/g, '__').length > 12) {
			this.setState({
				nameWarning: '服務名稱不能多於6個漢字或者12個字母',
			});
			return false;
		}
		// 图标验证
		...
		if (serviceType === 1 && clientStatus && !clientUrl) {
			...
			this.setState({
				clientUrlWarning: '請輸入在友空間桌面端打開時跳轉地址',
			});
			return false;
		}
		// 友空间
