...
class AddApp extends Component {
	...
	servicePublish = () => {
		...
		if (serviceType == 1 && serviceName === '') {
			this.setState({
				nameWarning: 'Please enter service name.',
			});
			return false;
		}
		if (serviceType == 1 && serviceName.trim().replace(/[^\x00-\xff]/g, '__').length > 12) {
			this.setState({
				nameWarning: 'Service name cannot be more than 6 Chinese chars or 12 letters.',
			});
			return false;
		}
		// 图标验证
		if (!serviceIcon) {
			this.setState({
				imgWarning: 'Please upload the Service icon.',
			});
			return false;
		}
		...
		if (serviceType === 1 && webStatus && !url) {
			...
			this.setState({
				urlWarning: 'Redirect URL for Web browser',
			});
			return false;
		}
		// pc客户端
		if (serviceType === 1 && clientStatus && !clientUrl) {
			...
			this.setState({
				clientUrlWarning: 'Redirect URL for YouZone PC version',
			});
			return false;
		}
		// 友空间
		if (typeof primitive === 'undefined') {
			...
			this.setState({
				selectWarning: 'Please select if it is a native App.',
			});
			return false;
		}
		// 20181009只对自建服务
		if (serviceType === 1 && phoneStatus && primitive && !iosDownAddr) {
			// this.setSelectType('phone');
			this.setState({
				iosWarning: 'Please enter IOSDownload Link',
			});
			return false;
		}
		if (serviceType === 1 && phoneStatus && primitive && !androidDownAddr) {
			// this.setSelectType('phone');
			this.setState({
				androidWarning: 'Please enter AndroidDownload Link',
			});
			return false;
		}
		...
		if (serviceType === 1 && phoneStatus && !primitive && !phoneUrl && !summerPackageId) {
			// this.setSelectType('phone');
			this.setState({
				appWarning: 'Mobile Homepage,summerIdChoose One',
			});
			return false;
		}
		...
		addService(allParams).then((payload) => {
			...
			let instance = openGlobalDialog({
				title: !!serviceId ? 'Modified successfully' : 'Created successfully',
				duration: 3,
				type: 'success',
				closable: true,
		...
		imgObj.onload = function () {
			if (imgObj.width !== imgObj.height || imgObj.width !== 120 || obj.size > 5000000) {
				openMess({
					...
					content: 'Size: 120*120; no more than 5M!',
					duration: 6,
					type: 'error',
					closable: false,
					style: { zIndex: 1711 }
