...
const handlers = {
	...
	openWin(data) {
		...
		if (!flag && store.getState().wrap.tabs.length >= 20) {
			openMess({
				title: "You can open up to 20 pages. Please close the pages you don't need.",
				duration: 2,
				type: 'error',
				closable: false,
			});
			...
			get('/service/getServiceByTenantIdAndServiceCode', {
				...
				if (!crossTenant) {
					openGlobalDialog({
						...
						content: 'You need to switch to the corresponding enterprise to view details. Switch?',
						btns: [
							{
								label: 'Switch',
								fun: () => {
